...
export function ResultsPanel({ outputs, inputs }: ResultsPanelProps) {
  const sectionLabel = 'text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-2';
  const card = 'bg-zinc-900 border border-zinc-800 rounded-xl p-4';
  const divider = <div className="border-t border-zinc-800 my-1" />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-zinc-500 text-xs mt-2 px-1">{outputs.totalStations} Station(en) konfiguriert</p>
      </div>
      ...
      <div>
        ...
        <div className={card}>
          <MetricRow label="Umsatz / Monat" value={outputs.umsatz_Mo} format="currency" variant="amber" />
          <div className="flex justify-between items-center py-2.5 transition-all duration-150">
            <span className="text-zinc-400 text-sm">Umsatz / Jahr</span>
          ...
          {divider}
          <p className="text-xs font-semibold uppercase tracking-widest text-zinc-600 mt-1 mb-0.5">PB Details</p>
      ...
      <div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          <p className="text-zinc-400 text-sm mb-1">Jährlicher Nettozufluss</p>
          <p className="text-3xl font-bold text-amber-400 tabular-nums transition-all duration-150">
            {formatCurrency(outputs.annualNetFlow)}
          </p>
          <p className="text-zinc-500 text-xs mt-2">nach MwSt, Gebühren &amp; Umsatzshare</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {/* Jahr 1 */}
          <div
            className={`bg-zinc-900 rounded-xl p-4 border transition-all duration-150 ${
              outputs.gewinn_Jahr1 >= 0 ? 'border-amber-500/30' : 'border-red-500/30'
            }`}
          >
            <p className={sectionLabel}>Jahr 1</p>
            <p
              className={`font-bold text-xl tabular-nums ${
                outputs.gewinn_Jahr1 >= 0 ? 'text-emerald-400' : 'text-red-400'
              }`}
            >
          ...
          {/* Jahr 2 */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 transition-all duration-150">
            <p className={sectionLabel}>Jahr 2</p>
            <p
              className={`font-bold text-xl tabular-nums ${
                outputs.gewinn_Jahr2 >= 0 ? 'text-emerald-400' : 'text-red-400'
              }`}
            >
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 transition-all duration-150">
            ...
            <p
              className={`font-bold text-xl tabular-nums ${
                outputs.gewinn_Jahr3 >= 0 ? 'text-emerald-400' : 'text-red-400'
              }`}
            >
        ...
        {/* Gesamtgewinn */}
        <div className="mt-3 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 flex justify-between items-center">
          <span className="text-zinc-400 text-sm font-semibold">Gesamtgewinn (3 Jahre):</span>
          <span
            className={`font-bold text-lg tabular-nums transition-all duration-150 ${
              outputs.gewinn_Gesamt >= 0 ? 'text-emerald-400' : 'text-red-400'
            }`}
          >
      ...
      <div>
        ...
        <div className={card}>
          <div className="flex justify-between items-center py-2.5 transition-all duration-150">
            <span className="text-zinc-400 text-sm">ROI Jahr 1</span>
            <span
              className={`font-semibold tabular-nums ${
                outputs.roi_Jahr1 >= 0 ? 'text-emerald-400' : 'text-red-400'
              }`}
            >
          ...
          <div className="flex justify-between items-center py-2.5 transition-all duration-150">
            <span className="text-zinc-400 text-sm">ROI 3 Jahre</span>
            <span
              className={`font-semibold tabular-nums ${
                outputs.roi_3Jahre >= 0 ? 'text-emerald-400' : 'text-red-400'
              }`}
            >
        ...
        <p className="text-xs text-zinc-500 mt-2 px-1">Basis: Maschinenkosten (Einstiegsgebühr exkludiert)</p>
      </div>
    </div>
  );
}
