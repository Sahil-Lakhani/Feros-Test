...
// ─── INVESTMENT ───────────────────────────────────────────────────────────────
export function calcMachineCost(qty_S, qty_M, qty_L, qty_XL) {
  return qty_S * 1000 + qty_M * 1200 + qty_L * 2000 + qty_XL * 3500;
}

export function calcTotalInvestment(machineCost) {
  return ENTRY_FEE + machineCost;
}

// ─── RENTAL REVENUE ───────────────────────────────────────────────────────────
// IMPORTANT: maxDaily does NOT cap rental revenue — it only affects incomePerLostPB
export function calcPricePerRental(hourlyRate, rentalHours) {
  return hourlyRate * rentalHours;
}

export function calcTotalRentals(totalStations, rentalsPerDay, openingDays) {
  return totalStations * rentalsPerDay * openingDays;
}

export function calcRentalRevenue(totalRentals, pricePerRental) {
  return totalRentals * pricePerRental;
}

// ─── LOST PB MECHANICS ────────────────────────────────────────────────────────
export function calcLostPBs(totalRentals, pbLostRate) {
  return totalRentals * pbLostRate;
}

// deposit can cap income: incomePerLostPB = min(deposit, maxDaily × timeoutDays + 15)
export function calcIncomePerLostPB(deposit, maxDaily, timeoutDays) {
  return Math.min(deposit, maxDaily * timeoutDays + REPLACEMENT_COST_PB);
}

export function calcNetGainPerLostPB(incomePerLostPB) {
  return incomePerLostPB - REPLACEMENT_COST_PB;
}

export function calcPBRefillGains(lostPBs, netGainPerLostPB) {
  return lostPBs * netGainPerLostPB;
}

// ─── ANNUAL CASH FLOW ─────────────────────────────────────────────────────────
// VAT is EXTRACTED from revenue (enthaltene MwSt) — formula: revenue × 0.19/1.19
export function calcVATExtracted(umsatz_Jahr) {
  return umsatz_Jahr * (VAT_RATE / (1 + VAT_RATE));
}

export function calcPartnerFee(umsatz_Jahr) {
  return umsatz_Jahr * PARTNER_FEE;
}

export function calcSystemFee(umsatz_Jahr) {
  return umsatz_Jahr * SYSTEM_FEE;
}

export function calcMerchantFee(umsatz_Jahr, merchantShare) {
  return umsatz_Jahr * merchantShare;
}

export function calcAnnualNetFlow(umsatz_Jahr, merchantShare) {
...
export function calcGewinnJahr1(annualNetFlow, totalInvestment) {
  return annualNetFlow - totalInvestment;
}

export function calcGewinnGesamt(annualNetFlow, totalInvestment) {
  return 3 * annualNetFlow - totalInvestment;
}

// ─── ROI ──────────────────────────────────────────────────────────────────────
// CRITICAL: denominator is machineCost ONLY — entry fee (€1,900) is excluded
export function calcROIJahr1(gewinn_Jahr1, machineCost) {
  if (machineCost === 0) return 0;
  return (gewinn_Jahr1 / machineCost) * 100;
}

export function calcROI3Jahre(gewinn_Gesamt, machineCost) {
  if (machineCost === 0) return 0;
